...
export const register = async (req: Request, res: Response) => {
  try {
    ...
    // Send verification email via Resend template
    ...
    const accessToken = signAccessToken({ userId: user.id, role: user.role, emailVerified: user.emailVerified });
    ...
    return res.status(201).json({
      ...
      data: { accessToken, refreshToken, user: { id: user.id, name: user.name, email: user.email, role: user.role, emailVerified: user.emailVerified } },
    });
  } catch (error: any) {
    if (error instanceof z.ZodError) {
      return res.status(400).json({ success: false, error: 'Validation error', issues: error.issues });
    }
...
export const resetPassword = async (req: Request, res: Response) => {
  try {
    ...
    if (!resetToken) return res.status(400).json({ success: false, message: "Invalid or expired reset code." });
    ...
    await prisma.$transaction([
      prisma.user.update({ where: { id: resetToken.userId }, data: { password_hash: newHashedPassword } }),
